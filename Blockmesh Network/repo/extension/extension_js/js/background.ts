...
let short_stop = false

async function get_short_stop() {
  console.log('get_short_stop running')
  try {
    const response = await fetch('https://feature-flags.blockmesh.xyz/extension/read-flag/extension_short_stop')
    if (response.ok) {
      const value = await response.text()
      short_stop = value === 'true'
    } else {
      short_stop = false
    }
  } catch (error: any) {
    console.error('get_short_stop error:', error)
    short_stop = false
  }
}

setInterval(async () => {
  await get_short_stop()
}, 60 * 60 * 1_000)
...
async function main_interval() {
  ...
  if (is_ws_enabled) {
    ...
    start_websocket().then(async () => {
      const interval = setInterval(async () => {
          if (short_stop) {
            console.log('short stop')
            clearInterval(interval)
