...
class ReclaimContentScript {
  ...
  checkIfOnCorrectPage() {
    ...
    // ⭐ ENHANCED: Provider-specific page detection ⭐
    const providerPagePatterns = {
      'github.com': ['/settings/profile', '/settings', '/profile'],
      'linkedin.com': ['/feed', '/in/', '/profile', '/mynetwork', '/messaging', '/jobs', '/learning'],
      'instagram.com': ['/', '/p/', '/stories/', '/reels/', '/explore/', '/direct/'],
      'twitter.com': ['/home', '/profile', '/user/'],
      'x.com': ['/home', '/profile', '/user/'], // Add X.com domain
      'gmail.com': ['/mail/', '/inbox/'],
      'mail.google.com': ['/mail/', '/inbox/']
    };

    // Check if current URL matches any provider-specific patterns
    for (const [domain, patterns] of Object.entries(providerPagePatterns)) {
      if (currentUrl.includes(domain)) {
        ...
        if (isOnCorrectPage) break;
      }
    }

    // ⭐ NEW: Enhanced Instagram and LinkedIn detection ⭐
    const providerName = this.providerData?.name?.toLowerCase() || '';
    
    // For Instagram - any page on instagram.com is valid
    if (providerName === 'instagram' && currentUrl.includes('instagram.com')) {
      isOnCorrectPage = true;
      addDebugLog('✅ Instagram page detected as valid', {
        currentUrl: currentUrl,
        providerName: providerName
      }, 'success');
    }
    
    // For LinkedIn - any page on linkedin.com is valid
    if (providerName === 'linkedin' && currentUrl.includes('linkedin.com')) {
      isOnCorrectPage = true;
      addDebugLog('✅ LinkedIn page detected as valid', {
        currentUrl: currentUrl,
        providerName: providerName
      }, 'success');
    }
  ...
  extractPageData() {
    ...
    // ⭐ ENHANCED: Provider-specific data extraction ⭐
    const currentUrl = window.location.href;
    const providerName = this.providerData?.name?.toLowerCase() || 'unknown';
    
    // Provider-specific extraction patterns
    const providerExtractionPatterns = {
      'github': {
        username: [
          () => document.querySelector('[data-testid="user-profile-name"]')?.textContent?.trim(),
          () => document.querySelector('meta[name="octolytics-actor-login"]')?.getAttribute('content'),
          () => {
            const titleMatch = document.title.match(/GitHub - ([^\s]+)/);
      ...
      'linkedin': {
        username: [
          ...
          () => {
            const urlMatch = currentUrl.match(/linkedin\.com\/in\/([^\/\s]+)/);
      ...
      'instagram': {
        username: [
          () => {
            const urlMatch = currentUrl.match(/instagram\.com\/([^\/\s]+)/);
            return urlMatch ? urlMatch[1] : null;
          },
          () => document.querySelector('meta[property="og:title"]')?.getAttribute('content')
        ],
        profile: [
          () => document.querySelector('h2._aacl._aacs._aact._aacx._aada')?.textContent?.trim()
        ]
      },
      'twitter': {
        username: [
          () => {
            const urlMatch = currentUrl.match(/twitter\.com\/([^\/\s]+)/);
            return urlMatch ? urlMatch[1] : null;
          },
          () => {
            const urlMatch = currentUrl.match(/x\.com\/([^\/\s]+)/);
            return urlMatch ? urlMatch[1] : null;
          },
          () => document.querySelector('meta[name="twitter:site"]')?.getAttribute('content')
        ],
        profile: [
          () => document.querySelector('h1[data-testid="UserName"]')?.textContent?.trim(),
          () => document.querySelector('div[data-testid="UserName"]')?.textContent?.trim(),
          () => document.querySelector('span[data-testid="UserName"]')?.textContent?.trim()
        ]
      },
      'gmail': {
        email: [
          ...
          () => document.querySelector('div[data-email]')?.getAttribute('data-email')
        ]
      }
    };
    
    // Apply provider-specific extraction
    const patterns = providerExtractionPatterns[providerName] || providerExtractionPatterns['github'];
    if (patterns) {
      Object.entries(patterns).forEach(([key, extractors]) => {
    ...
    addDebugLog('📄 Extracted page data', {
      ...
      providerName: providerName,
      hasUsername: !!extractedData.username,
  ...
  createComprehensiveExtractionConfig() {
    ...
    // ⭐ ENHANCED: Add provider-specific extraction patterns ⭐
    const providerName = this.providerData.name?.toLowerCase() || 'unknown';
    ...
    const comprehensiveResponseRedactions = [
      // Generic XPath patterns for common elements
      {
      ...
      {
        xPath: '//input[@name="user"]/@value',
        type: 'xpath'
      },
      // Generic regex patterns for common data
      {
        regex: '"username":"([^"]+)"',
        type: 'regex'
      },
    ...
    const providerPatterns = providerSpecificPatterns[providerName] || providerSpecificPatterns['github'];
    comprehensiveResponseRedactions.push(...providerPatterns);
    ...
    addDebugLog('📝 Created comprehensive extraction configuration', {
      ...
      providerPatternsCount: providerPatterns.length,
  ...
  triggerTargetRequestNow(expectedUrl) {
    ...
    // ⭐ ENHANCED: Provider-specific trigger URLs ⭐
    const providerName = this.providerData?.name?.toLowerCase() || 'unknown';
    const providerTriggerUrls = {
    ...
    // ⭐ ENHANCED: Handle X.com domain for Twitter ⭐
    let targetUrl = providerTriggerUrls[providerName] || expectedUrl;
    
    // If we're on X.com and the provider is Twitter, use X.com URL
    if (providerName === 'twitter' && window.location.href.includes('x.com')) {
      targetUrl = 'https://x.com/home';
    }
    
    // ⭐ NEW: Enhanced Instagram and LinkedIn trigger URLs ⭐
    if (providerName === 'instagram') {
      // For Instagram, trigger multiple requests to capture user data
      const instagramUrls = [
      ...
      // Use the first URL as primary, but log all options
      targetUrl = instagramUrls[0];
      addDebugLog('📸 Instagram trigger URLs available', {
        primaryUrl: targetUrl,
        allUrls: instagramUrls,
        currentUrl: window.location.href
      }, 'info');
    }
    
    if (providerName === 'linkedin') {
      // For LinkedIn, trigger multiple requests to capture user data
      const linkedinUrls = [
      ...
      // Use the first URL as primary, but log all options
      targetUrl = linkedinUrls[0];
      addDebugLog('💼 LinkedIn trigger URLs available', {
        primaryUrl: targetUrl,
        allUrls: linkedinUrls,
        currentUrl: window.location.href
      }, 'info');
    }
    
    addDebugLog('🎯 MANUAL TRIGGER: Using target URL', {
      providerName: providerName,
      targetUrl: targetUrl,
      originalExpectedUrl: expectedUrl
    }, 'info');
    ...
    // ⭐ NEW: Trigger multiple requests for Instagram and LinkedIn ⭐
    if (providerName === 'instagram' || providerName === 'linkedin') {
      this.triggerMultipleProviderRequests(providerName);
    }
  ...
  filterInterceptedRequests() {
    if (!this.providerData || !this.providerData.requestData) {
      ...
      // ⭐ NEW: Enhanced Instagram and LinkedIn immediate trigger ⭐
      const providerName = this.providerData?.name?.toLowerCase() || '';
      
      if (providerName === 'instagram' && window.location.href.includes('instagram.com')) {
        addDebugLog('📸 ON INSTAGRAM PAGE - Triggering Instagram verification', {
          hasProviderData: !!this.providerData,
          currentUrl: window.location.href,
          providerName: providerName
        }, 'success');
        
        // Trigger Instagram-specific requests
        this.triggerMultipleProviderRequests('instagram');
      }
      
      if (providerName === 'linkedin' && window.location.href.includes('linkedin.com')) {
        addDebugLog('💼 ON LINKEDIN PAGE - Triggering LinkedIn verification', {
          hasProviderData: !!this.providerData,
          currentUrl: window.location.href,
          providerName: providerName
        }, 'success');
        
        // Trigger LinkedIn-specific requests
        this.triggerMultipleProviderRequests('linkedin');
      }
    ...
    for (const [key, linkedData] of this.linkedRequestResponses.entries()) {
      ...
      // ⭐ NEW: Enhanced Instagram and LinkedIn handling - bypass strict criteria for these providers ⭐
      const currentProviderName = this.providerData?.name?.toLowerCase() || '';
      const isInstagram = currentProviderName === 'instagram' && formattedRequest.url.includes('instagram.com');
      const isLinkedIn = currentProviderName === 'linkedin' && formattedRequest.url.includes('linkedin.com');
      
      if ((isInstagram || isLinkedIn) && !this.filteredRequests.includes(key)) {
        // For Instagram and LinkedIn, accept any API request that contains user data
        const hasUserData = formattedRequest.responseText && (
          formattedRequest.responseText.includes('"username"') ||
          formattedRequest.responseText.includes('"publicIdentifier"') ||
          formattedRequest.responseText.includes('"full_name"') ||
          formattedRequest.responseText.includes('"firstName"') ||
          formattedRequest.responseText.includes('"lastName"') ||
          formattedRequest.responseText.includes('"screen_name"') ||
          formattedRequest.responseText.includes('"name"') ||
          formattedRequest.responseText.includes('"email"')
        );
        
        if (hasUserData) {
          addDebugLog('🎯 INSTAGRAM/LINKEDIN API REQUEST ACCEPTED!', {
            url: formattedRequest.url,
            method: formattedRequest.method,
          ...
          loggerService.log({
            message: `Instagram/LinkedIn API request accepted: ${formattedRequest.method} ${formattedRequest.url}`,
            type: LOG_TYPES.CONTENT,
            sessionId: this.sessionId,
            providerId: this.httpProviderId,
            appId: this.appId
          ...
          // Create a mock criteria object for background processing
          const mockCriteria = {
            url: formattedRequest.url,
            method: formattedRequest.method,
            urlType: 'EXACT'
          ...
          // Send to background script for cookie fetching and claim creation
          this.sendFilteredRequestToBackground(formattedRequest, mockCriteria, this.providerData.loginUrl);
        }
      }
    }
    ...
    if (this.filteredRequests.length >= this.providerData.requestData.length) {
      ...
      this.linkedRequestResponses.clear();
          ...
          // ⭐ NEW: Handle Instagram and LinkedIn cases where we might not have matching criteria ⭐
      const providerNameForCleanup = this.providerData?.name?.toLowerCase() || '';
      const isInstagramForCleanup = providerNameForCleanup === 'instagram';
      const isLinkedInForCleanup = providerNameForCleanup === 'linkedin';
    
    if ((isInstagramForCleanup || isLinkedInForCleanup) && this.filteredRequests.length > 0) {
      loggerService.log({
        message: `Found ${this.filteredRequests.length} Instagram/LinkedIn API requests, stopping filtering and cleaning up resources`,
        type: LOG_TYPES.CONTENT,
        sessionId: this.sessionId,
        providerId: this.httpProviderId,
        appId: this.appId
  ...
  // ⭐ NEW: Trigger multiple requests for Instagram and LinkedIn ⭐
  triggerMultipleProviderRequests(providerName) {
    addDebugLog('🚀 MULTI-TRIGGER: Triggering multiple requests for provider', {
      providerName: providerName,
      timestamp: new Date().toISOString()
    }, 'info');
    ...
    if (providerName === 'instagram') {
      ...
      instagramUrls.forEach((url, index) => {
        setTimeout(() => {
          addDebugLog(`📸 Instagram request ${index + 1}: ${url}`, {
            url: url,
            index: index + 1,
            total: instagramUrls.length
          }, 'info');
          ...
          fetch(url, {
            ...
            addDebugLog(`✅ Instagram request ${index + 1} successful`, {
              url: url,
              status: response.status,
              index: index + 1
            }, 'success');
          }).catch(error => {
            addDebugLog(`❌ Instagram request ${index + 1} failed`, {
              url: url,
              error: error.message,
              index: index + 1
            }, 'error');
    ...
    if (providerName === 'linkedin') {
      ...
      linkedinUrls.forEach((url, index) => {
        setTimeout(() => {
          addDebugLog(`💼 LinkedIn request ${index + 1}: ${url}`, {
            url: url,
            index: index + 1,
          ...
          fetch(url, {
            ...
            addDebugLog(`✅ LinkedIn request ${index + 1} successful`, {
              url: url,
              status: response.status,
              index: index + 1
            }, 'success');
          }).catch(error => {
            addDebugLog(`❌ LinkedIn request ${index + 1} failed`, {
              url: url,
              error: error.message,
              index: index + 1
            }, 'error');
...
window.testWootzZKProof = function() {
  ...
  return 'Wootz API test initiated';
};
