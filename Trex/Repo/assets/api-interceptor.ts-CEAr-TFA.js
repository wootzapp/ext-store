...
class h {
  ...
  injectApiInterceptor() {
    ...
    try {
      ...
      t.onload = () => {
        ...
        console.log("🔄 [FLOW-TRACE] Sending initialization message to inject script...");
        window.postMessage(
          {
      ...
      if (document.documentElement) {
        console.log("🔄 [FLOW-TRACE] Appending script to document.documentElement...");
        document.documentElement.appendChild(t);
      } else {
        console.log("⚠️ [FLOW-TRACE] document.documentElement not ready - using MutationObserver...");
        const i = new MutationObserver(() => {
          if (document.documentElement) {
            console.log("✅ [FLOW-TRACE] document.documentElement ready - appending script...");
            document.documentElement.appendChild(t);
            i.disconnect();
          }
        });
  ...
  async handleTwitterRequestData(e, t = 0) {
    ...
    console.log("🎉 [FLOW-TRACE] SUCCESS! Flow reached handleTwitterRequestData function");
    ...
    if (e) {
      ...
      console.log("📐 [FLOW-TRACE] Response length:", e.response ? e.response.length : 0);
    ...
    try {
      await new Promise((i, n) => {
        ...
        console.log("📏 [FLOW-TRACE] Data Size:", JSON.stringify(e).length, "chars");
        
        chrome.runtime.sendMessage(
          { type: a.TwitterSendApiData, data: e },
          (I) => {
            if (chrome.runtime.lastError) {
              console.error("❌ [FLOW-TRACE] Service Worker Error:", chrome.runtime.lastError);
              n(chrome.runtime.lastError);
              return;
            }
            console.log("✅ [FLOW-TRACE] Service Worker Response:", I);
            console.log("🎉 [FLOW-TRACE] Message successfully sent to service worker!");
            i();
          }
        );
  ...
  async init(e) {
    ...
    console.log("🔍 [FLOW-TRACE] URL validation passed:", isTwitterUrl || isXUrl);
    
    if (!isTwitterUrl && !isXUrl) {
      console.log("❌ [FLOW-TRACE] URL validation FAILED - not Twitter/X website");
      return;
    }
    ...
    console.log("🚀 [FLOW-TRACE] Starting Twitter interceptor initialization...");
    
    try {
      console.log("🔄 [FLOW-TRACE] Step 1: Calling this.cleanup()...");
      this.cleanup();
      
      console.log("🔄 [FLOW-TRACE] Step 2: Calling this.injectApiInterceptor()...");
      this.injectApiInterceptor();
      ...
      this.messageHandler = (t) => {
        ...
        console.log("🔍 [FLOW-TRACE] Is Twitter API data:", t.data.type === "TREX_TWITTER_API_DATA");
        ...
        if (content) {
          ...
          console.log("📏 [FLOW-TRACE] Response Size:", content.response ? content.response.length : 0, "chars");
      ...
      console.log("🔄 [FLOW-TRACE] Step 5: Sending initialization message to inject script...");
      window.postMessage(
        {
          type: "TREX_INITIALIZE_TWITTER_INTERCEPTOR",
          timestamp: Date.now(),
...
const A = () => {
    ...
    console.log("📍 [FLOW-TRACE] Function A() called - API拦截器 开始初始化API拦截器...");
  ...
  P = () => {
    ...
    console.log("📡 [FLOW-TRACE] Sending CheckLoginStatus message to background...");
    ...
    chrome.runtime.sendMessage({ type: a.CheckLoginStatus }, (o) => {
      ...
      if (e) {
        ...
        console.log("🚀 [FLOW-TRACE] User is logged in - calling d() with URL...");
        d(t);
      } else {
        console.log("❌ [FLOW-TRACE] User NOT logged in - skipping API interceptor initialization");
      }
    });
  },
  b = () => {
  ...
  d = (o) => {
    ...
    if (isTwitterSite) {
      console.log("✅ [FLOW-TRACE] Twitter/X site detected - checking permissions...");
      console.log("📡 [FLOW-TRACE] Getting trex_twitter_permission from storage...");
      ...
      chrome.storage.local.get("trex_twitter_permission", (e) => {
        ...
        console.log("🔍 [FLOW-TRACE] Boolean evaluation:", t === !0 || t === "true");
        
        if (t === !0 || t === "true") {
          console.log("✅ [FLOW-TRACE] 🎉 TWITTER PERMISSION ENABLED!");
          console.log("🚀 [FLOW-TRACE] Calling c.init() to initialize Twitter interceptor...");
          c.init(o);
        } else {
          console.log("❌ [FLOW-TRACE] 🚫 TWITTER PERMISSION DISABLED OR MISSING");
          console.log("⚠️ [FLOW-TRACE] This is why Twitter API is not being intercepted!");
        }
      });
    } else {
      console.log("❌ [FLOW-TRACE] Not a Twitter/X site - skipping Twitter check");
    }
    ...
    if (isYouTubeSite) {
      console.log("✅ [FLOW-TRACE] YouTube site detected - checking permissions...");
      chrome.storage.local.get("trex_youtube_permission", (e) => {
        const t = e.trex_youtube_permission;
        console.log("🔥 [FLOW-TRACE] YouTube permission:", t);
        
        if (t === !0 || t === "true") {
          console.log("✅ [FLOW-TRACE] YouTube permission enabled - initializing...");
          s.init(o);
        } else {
      ...
      console.log("❌ [FLOW-TRACE] Not a YouTube site - skipping YouTube check");
    }
  };
...
chrome.storage.local.get(null, (result) => {
  ...
  console.log("🐦 [FLOW-TRACE] Twitter permission:", result.trex_twitter_permission);
  console.log("📺 [FLOW-TRACE] YouTube permission:", result.trex_youtube_permission);
