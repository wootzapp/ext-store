...
class BackgroundTwitterAgent {
  ...
  async initializePersistentState() {
    try {
      ...
      if (this.isRunning && data.agentConfig) {
        ...
        const activePlatform = data.activePlatform || "twitter";
        await this.startAgent(data.agentConfig, activePlatform);
        ...
        if (syncData.agentConfig) {
          console.log(
            "Background: Found config in sync storage, restarting agent"
          );
          const activePlatform = data.activePlatform || "twitter";
          await this.startAgent(syncData.agentConfig, activePlatform);
        } else {
        ...
        console.log(
          "Background: Agent is running, ensuring keep-alive mechanisms"
        );
      }
      ...
      if (data.pendingTweet && this.isRunning) {
        console.log(
          "Background: Found pending tweet from previous session, attempting to post"
        );
        setTimeout(async () => {
          try {
            const postResult = await this.postTweetViaTab(
              data.pendingTweet.content
            );
            if (postResult.success) {
              await chrome.storage.local.remove(["pendingTweet"]);
              console.log(
                "Background: Pending tweet from previous session posted successfully"
              );
            }
          } catch (error) {
            console.error(
              "Background: Error posting pending tweet from previous session:",
              error
            );
          }
      ...
      if (data.currentApiAttempt && this.isRunning) {
        console.log(
          "Background: Found incomplete API attempt from previous session, continuing..."
        );
  ...
  setupMessageHandlers() {
        ...
        switch (request.action) {
          ...
          case "GENERATE_AND_POST_TWEET":
            try {
              ...
              console.error(
                "Background: Error in GENERATE_AND_POST_TWEET:",
                error
              );
              sendResponse({
                success: false,
          ...
          case "CHECK_INSTAGRAM_LOGIN":
            try {
              console.log("Background: CHECK_INSTAGRAM_LOGIN received");
              const result = await this.performInitialLoginCheckInstagram();
              sendResponse(result);
            } catch (error) {
              console.error(
                "Background: Error in CHECK_INSTAGRAM_LOGIN:",
                error
              );
              sendResponse({
                success: false,
          ...
          case "GENERATE_AND_POST_INSTAGRAM":
            // const instagramResult = await this.generateAndPostInstagram();
            // sendResponse(instagramResult);
            console.log("Background: GENERATE_AND_POST_INSTAGRAM received");
            break;
          ...
          case "CLAUDE_GENERATE":
            const claudeResult = await this.claudeGenerate(
              request.apiKey,
              request.topic
            );
          ...
          case "OPENAI_GENERATE":
            const openaiResult = await this.openaiGenerate(
              request.apiKey,
              request.topic
            );
          ...
          case "GEMINI_GENERATE":
            const geminiResult = await this.geminiGenerate(
              request.apiKey,
              request.topic
            );
          ...
          case "OPEN_PROFILE_TAB":
            try {
              console.log("Background: OPEN_PROFILE_TAB received");
              const result = await this.openProfileTab(request.url);
              sendResponse(result);
            } catch (error) {
              console.error("Background: Error in OPEN_PROFILE_TAB:", error);
              sendResponse({
                success: false,
                error: error.message,
              });
          ...
          case "LOGOUT_INSTAGRAM":
            console.log("Background: LOGOUT_INSTAGRAM received");
            return new Promise((resolve, reject) => {
              // First destroy any existing Instagram WebContents to prevent conflicts
              chrome.wootz.destroyBackgroundWebContents(3, () => {
                setTimeout(() => {
                  chrome.wootz.createBackgroundWebContents(
                    ...
                    (result) => {
                      if (result.success) {
                        console.log(
                          "Background: Created Instagram WebContents for logout: 3"
                        );
                        ...
                        const logoutTimeout = setTimeout(() => {
                          if (!isResolved) {
                            console.log("Background: Logout timeout");
                            isResolved = true;
                            if (messageListener) {
                            ...
                            chrome.wootz.destroyBackgroundWebContents(
                              3,
                              () => {}
                            );
                        ...
                        messageListener = async (request, sender) => {
                          if (
                            request.action === "CONTENT_SCRIPT_READY" &&
                            !isResolved
                          ) {
                            try {
                              // Add small delay to ensure content script is fully ready
                              ...
                              console.error(
                                "Background: Error during Instagram logout:",
                                error
                              );
                              if (!isResolved) {
                                clearTimeout(logoutTimeout);
                        ...
                        console.error(
                          "Background: Failed to create Instagram WebContents for logout:",
                          result.error
                        );
    ...
    chrome.alarms.onAlarm.addListener(async (alarm) => {
      if (alarm.name === ALARMS.TWEET_GENERATOR) {
        ...
        try {
          ...
          // Only proceed if this is a Twitter agent
          if (localData.activePlatform !== "twitter") {
            console.log(
              "Background: Not a Twitter agent, skipping tweet generation alarm"
            );
            return;
          }
          ...
          if (!localData.isRunning && config) {
            ...
            await this.startAgent(
              config,
              localData.activePlatform || "twitter"
            );
            return;
          }
          ...
          if (localData.isRunning && !localData.isProcessing) {
            ...
            console.log(
              "Background: Agent not running or already processing, skipping alarm"
            );
          }
        } catch (error) {
        ...
        try {
          const { isRunning, activePlatform } = await chrome.storage.local.get([
            "isRunning",
            "activePlatform",
          ]);
          ...
          if (!isRunning) {
            console.log(
              "Background: Wake-up check - agent not running, skipping all operations"
            );
            return;
          }

          // Only proceed with tweet checks for Twitter agents
          if (activePlatform !== "twitter") {
            console.log(
              "Background: Wake-up check - not a Twitter agent, skipping tweet operations"
            );
            return;
          }

          const { currentApiAttempt } = await chrome.storage.local.get([
            "currentApiAttempt",
          ]);

          if (!currentApiAttempt) {
            console.log(
              "Background: Wake-up check - agent running, checking tweet schedule"
            );
            await this.checkAndPostTweet();
          } else {
            console.log(
              "Background: Wake-up check - API attempt in progress, continuing..."
            );
          }
        } catch (error) {
          console.error(
            "Background: Error handling wake-up check alarm:",
            error
          );
        }
      }
    ...
    chrome.runtime.onStartup.addListener(async () => {
      ...
      if (data.isRunning && data.activePlatform === "twitter") {
        await this.checkAndPostTweet();
      }
    });
  }

  async openProfileTab(url) {
    console.log("Background: Opening profile tab via WebContents:", url);

    // Create Instagram WebContents with profile URL
    return new Promise((resolve, reject) => {
      // First destroy any existing Instagram WebContents to prevent conflicts
      chrome.wootz.destroyBackgroundWebContents(3, () => {
        setTimeout(() => {
          chrome.wootz.createBackgroundWebContents(3, url, (result) => {
            if (result.success) {
              console.log(
                "Background: Created Instagram WebContents for profile check: 3"
              );
              ...
              const checkTimeout = setTimeout(async () => {
                if (!isResolved) {
                  console.log("Background: Profile check timeout");
                  isResolved = true;
                  if (messageListener) {
              ...
              messageListener = async (request, sender) => {
                if (request.action === "CONTENT_SCRIPT_READY" && !isResolved) {
                  try {
                    console.log(
                      "Background: Instagram content script ready, checking profile..."
                    );
                    ...
                    if (!isResolved) {
                      ...
                      // Destroy WebContents after getting response
                      chrome.wootz.destroyBackgroundWebContents(3, () => {});

                      console.log(
                        "Background: Instagram profile check complete:",
                        profileResponse
                      );
                      resolve({
                        success: true,
                        loggedIn: profileResponse.loggedIn,
                      });
                    }
                  } catch (error) {
                    console.error(
                      "Background: Error during Instagram profile check:",
                      error
                    );
                    if (!isResolved) {
                      clearTimeout(checkTimeout);
              ...
              console.error(
                "Background: Failed to create Instagram WebContents for profile check:",
                result.error
              );
              reject(new Error(result.error));
            }
  ...
  async startAgent(config, platform = "twitter") {
    try {
      console.log(
        `Background: Starting ${platform} agent with config:`,
        config
      );
      ...
      if (platform === "twitter") {
        // Twitter agent logic
        const interval =
          config.twitter?.settings?.interval || config.settings?.interval || 30;
        ...
        // Perform initial login check before the first interval
        console.log(
          "Background: Performing initial Twitter login check at startup..."
        );
        const initialLoginResult = await this.performInitialLoginCheck();
        console.log(
          "Background: Initial Twitter login check result:",
          initialLoginResult
        );
        ...
        if (!initialLoginResult.success) {
          console.log(
            "Background: Initial Twitter login check failed, but continuing with agent startup"
          );
          console.log(
            "Background: Regular interval flow will handle login attempts as needed"
          );
        }
        ...
        console.log(
          `Background: Twitter agent started with ${interval} minute intervals`
        );
        return { success: true, message: "Twitter agent started successfully" };
      } else if (platform === "instagram") {
        // Instagram agent logic - only login check for now
        console.log(
          "Background: Starting Instagram agent (login check only)..."
        );
        ...
        // Perform Instagram login check
        console.log("Background: Performing Instagram login check...");
        const initialLoginResultInstagram =
          await this.performInitialLoginCheckInstagram();
        console.log(
          "Background: Instagram login check result:",
          initialLoginResultInstagram
        );

        if (initialLoginResultInstagram.success) {
          console.log(
            "Background: Instagram agent started successfully (login check completed)"
          );
          return {
            success: true,
            message:
              "Instagram agent started successfully (login check completed)",
            loginResult: initialLoginResultInstagram,
          };
        } else {
          console.log(
            "Background: Instagram login check failed, but agent started"
          );
          return {
            success: true,
            message: "Instagram agent started (login check failed)",
            loginResult: initialLoginResultInstagram,
          };
        }
      }
    } catch (error) {
      console.error(`Background: Error starting ${platform} agent:`, error);
      await this.stopAgent();
        ...
        config: config.agentConfig
          ? {
              hasTwitterCredentials: !!(
                config.agentConfig.twitter?.username &&
                config.agentConfig.twitter?.password
              ),
              hasInstagramCredentials: !!(
                config.agentConfig.instagram?.username &&
                config.agentConfig.instagram?.password
              ),
              twitterTopicsCount:
                config.agentConfig.twitter?.topics?.length || 0,
              instagramTopicsCount:
                config.agentConfig.instagram?.topics?.length || 0,
              twitterInterval: config.agentConfig.twitter?.settings?.interval,
              instagramInterval:
                config.agentConfig.instagram?.settings?.interval,
              aiModel: aiModel,
  ...
  async updateConfig(config) {
    try {
      ...
      console.log("Background: Config updated successfully", config);
  ...
  async postTweetViaTab(content, retryCount = 0, maxAttempts = 2) {
    try {
      ...
      if (
        !config.agentConfig?.twitter?.username ||
        !config.agentConfig?.twitter?.password ||
        !config.agentConfig?.twitter?.email
      ) {
        console.error(
          "Background: Twitter credentials not configured (requires username, password, and email)"
        );
        return {
          success: false,
          error:
            "Twitter credentials not configured (requires username, password, and email)",
          posted: false,
        };
      }
      ...
      if (!loginStatus.success || !loginStatus.loggedIn) {
        console.log(
          "Background: Not logged in, checking login status again..."
        );
      ...
      return new Promise((resolve) => {
        ...
        const messageListener = async (request) => {
          ...
          if (request.action === "CONTENT_SCRIPT_READY" && !hasReceivedReady) {
            ...
            console.log(
              "Background: Content script ready, sending tweet content..."
            );
            ...
            if (tweetResult.success && tweetResult.posted) {
              if (
                ...
                console.log(
                  "Background: Tweet posted successfully with verification:",
                  {
                    contentCleared:
                      tweetResult.verificationDetails?.contentCleared,
                ...
                console.warn(
                  "Background: Tweet marked as posted but without verification details"
                );
                tweetResult.success = false;
                tweetResult.message =
                  "Tweet status uncertain - no verification";
              }
            } else {
  ...
  async generateAndPostTweet() {
    try {
      ...
      const topics = agentConfig.twitter?.topics || agentConfig.topics || [];
      ...
      if (currentApiAttempt) {
        console.log(
          "Background: API attempt already in progress, waiting for completion..."
        );
      ...
      const { isRunning: stillRunning } = await chrome.storage.local.get([
        "isRunning",
      ]);
      if (!stillRunning) {
        console.log(
          "Background: Agent stopped during API call, aborting tweet posting"
        );
      ...
      if (result && result.success) {
        ...
        if (postResult.success && postResult.posted) {
          ...
          console.error(
            "Background: Scheduled tweet posting failed or unverified:",
            {
        ...
        console.error(
          "Failed to generate scheduled tweet:",
      ...
      const checkCompletion = async () => {
        try {
          ...
          if (!currentApiAttempt) {
            ...
            const { lastApiResult } = await chrome.storage.local.get([
              "lastApiResult",
            ]);
            if (lastApiResult) {
  ...
  async generateAndPostTweetManual() {
    try {
      ...
      console.log("Background: AI configuration:", {
        ...
        availableModels: config.agentConfig.ai?.apiKeys
          ? Object.keys(config.agentConfig.ai.apiKeys)
          : [],
      });
      ...
      const topics =
        config.agentConfig.twitter?.topics || config.agentConfig.topics || [];
      if (!topics || topics.length === 0) {
        console.error("Background: No Twitter topics configured");
        return { success: false, error: "No Twitter topics configured" };
      }

      const randomTopic = topics[Math.floor(Math.random() * topics.length)];

      console.log(
        "Background: Generating tweet about:",
        randomTopic,
        "using",
        aiModel
      );
      ...
      if (result && result.success) {
        ...
        console.error(
          "Background: Failed to generate manual tweet:",
          result?.error || "Unknown error"
        );
        return result || { success: false, error: "Failed to generate tweet" };
      }
      ...
      try {
        ...
        const response = await fetch("https://api.anthropic.com/v1/messages", {
          ...
          body: JSON.stringify({
            ...
            messages: [
              {
                role: "user",
                content: this.tweetPrompts[
                  Math.floor(Math.random() * this.tweetPrompts.length)
                ].replace("topic", topic),
              },
            ],
          }),
        ...
        if (response.status === 529) {
          ...
          if (attempt < retries) {
            ...
            console.log(
              `API overloaded, waiting ${backoffDelay}ms before retry...`
            );
        ...
        if (tweet.length > 280) {
          ...
          console.log(
            "Tweet is over 280 characters, keeping only 280 characters:",
            tweet
          );
        }
      ...
      try {
        ...
        if (response.status === 429) {
          ...
          if (attempt < retries) {
            ...
            console.log(
              `API rate limited, waiting ${backoffDelay}ms before retry...`
            );
            await new Promise((resolve) => setTimeout(resolve, backoffDelay));
            continue;
          }
  ...
  async performInitialLoginCheck() {
    try {
      ...
      if (
        ...
        console.log(
          "Background: Twitter credentials not configured, skipping initial login check"
        );
  ...
  // NEW: Method to perform initial Instagram login check at startup
  async performInitialLoginCheckInstagram() {
    try {
      console.log("Background: Performing initial Instagram login check...");
      ...
      if (
        ...
        console.log(
          "Background: Instagram credentials not configured, skipping initial login check"
        );
        return {
          success: true,
          message: "No Instagram credentials configured",
        };
      }

      // Check login status
      const loginStatus = await this.checkLoginStatusInWebContentsInstagram();

      if (loginStatus.success && loginStatus.loggedIn) {
        console.log("Background: Already logged in to Instagram");
        return {
          success: true,
      ...
      // Perform login if needed
      const loginResult = await this.performLoginForInstagram();
      return {
        success: loginResult.success,
        message: loginResult.message || loginResult.error,
        loggedIn: loginResult.loggedIn,
      ...
      console.error(
        "Background: Error in performInitialLoginCheckInstagram:",
        error
      );
      return {
        success: false,
  ...
  async checkLoginStatusInWebContents() {
    try {
      ...
      console.error(
        "Background: Error in checkLoginStatusInWebContents:",
        error
      );
      // await this.destroyTwitterWebContents();
      return {
  ...
  async checkLoginStatusInWebContentsInstagram() {
    try {
      console.log(
        "Background: Checking Instagram login status via WebContents..."
      );

      // Create Instagram WebContents
      await this.createInstagramWebContents();
      ...
      return new Promise((resolve) => {
        ...
        const checkTimeout = setTimeout(async () => {
          if (!isResolved) {
            console.log("Background: Instagram login check timeout");
            isResolved = true;
            if (messageListener) {
              chrome.runtime.onMessage.removeListener(messageListener);
            }
            await this.destroyInstagramWebContents();
            resolve({
              success: false,
              error: "Instagram login check timeout",
              loggedIn: false,
            });
          }
        }, 30000); // Increased timeout to 30 seconds
        ...
        messageListener = async (request, sender) => {
          if (request.action === "CONTENT_SCRIPT_READY" && !isResolved) {
            try {
              console.log(
                "Background: Instagram content script ready, checking login status..."
              );
              ...
              if (!isResolved) {
                ...
                console.log(
                  "Background: Instagram login check complete:",
                  loginResponse
                );
                resolve({
                  success: true,
              ...
              console.error(
                "Background: Error during Instagram login check:",
                error
              );
              if (!isResolved) {
                clearTimeout(checkTimeout);
      ...
      console.error(
        "Background: Error in checkLoginStatusInWebContentsInstagram:",
        error
      );
      await this.destroyInstagramWebContents(); // Cleanup on error
      return {
        success: false,
        error: error.message,
        loggedIn: false,
  ...
  async performLoginForInstagram() {
    try {
      console.log(
        "Background: Starting Instagram login process in visible tab..."
      );
        ...
        const messageListener = async (request, sender, sendResponse) => {
          ...
          if (request.action === "CONTENT_SCRIPT_READY" && !hasStartedLogin) {
            ...
            chrome.tabs.sendMessage(tab.id, {
              ...
              credentials: {
                username: config?.agentConfig?.instagram?.username,
                password: config?.agentConfig?.instagram?.password,
              },
            });
            ...
            resolve({
              success: request.result.success,
              message: request.result.success
                ? "Login successful"
                : "Login failed",
      ...
      console.error("Background: Error in performLoginForInstagram:", error);
      return {
        success: false,
        error: error.message,
        loggedIn: false,
    ...
    return new Promise((resolve, reject) => {
      chrome.wootz.createBackgroundWebContents(
        ...
        (result) => {
          if (result.success) {
            console.log(
              "Background: Created Twitter WebContents:",
              this.webContentsId
            );
            resolve(true);
          } else {
            console.error(
              "Background: Failed to create WebContents:",
              result.error
            );
            reject(new Error(result.error));
          }
  ...
  async createInstagramWebContents() {
    return new Promise((resolve, reject) => {
      // First destroy any existing Instagram WebContents to prevent conflicts
      chrome.wootz.destroyBackgroundWebContents(3, () => {
        // Wait a bit for cleanup, then create new WebContents
        setTimeout(() => {
          chrome.wootz.createBackgroundWebContents(
            3,
            ...
            (result) => {
              if (result.success) {
                console.log("Background: Created Instagram WebContents: 3");
                resolve(true);
              } else {
                console.error(
                  "Background: Failed to create Instagram WebContents:",
                  result.error
                );
                reject(new Error(result.error));
              }
      ...
      chrome.wootz.destroyBackgroundWebContents(
        ...
        (result) => {
          if (result.success) {
            ...
            console.error(
              "Background: Error destroying WebContents:",
              result.error
            );
          }
  ...
  async destroyInstagramWebContents() {
    return new Promise((resolve) => {
      chrome.wootz.destroyBackgroundWebContents(3, (result) => {
        if (result.success) {
          console.log("Background: Destroyed Instagram WebContents");
        } else {
          console.error(
            "Background: Error destroying Instagram WebContents:",
            result.error
          );
        }
        resolve(result.success);
  ...
  async isLoginCheckValid() {
    try {
      ...
      const isValid =
        Date.now() - data.lastLoginCheck.timestamp < this.loginCheckValidityMs;
